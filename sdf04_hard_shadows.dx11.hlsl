...
SdfResult scene(SceneData data, float3 p, bool renderLights = false)
{
  ...
  if (renderLights)
  {
    ...
    sceneRes = op_union(sceneRes,
                        sd_sphere(pLight, 0.1, make_material(float4(data.mainLight.color, 1), 0, float4(data.mainLight.color, 1))));
  }
  return sceneRes;
}
...
{
  float accumDist = 0.05;
  float3 currentPos = pos + dir * accumDist;
  for (int i = 0; i < MAX_ITER; i++)
  {
    SdfResult current = scene(sceneData, currentPos);
    if (current.dist < 1e-3)
    {
      // Hit something, is in shadow
      return 0.2f;
    }
    else
    {
      accumDist += current.dist;
      // Exceeded max distance without hitting, anything, exit with no occlusion
      if (accumDist >= maxDist) return 1.0f;
      
      currentPos += dir * current.dist;
    }
  }  
  return 1.0f;
}
...
float4 compute_lighting(SceneData sceneData, float3 pos, float3 normal, SdfResult r)
{
  ...
  // Shadowcast
  float occluded = raymarch_to_light(sceneData, pos, toLight, distanceToLight);
  
  return float4((sceneData.ambientLight + occluded * dt * sceneData.mainLight.color * sceneData.mainLight.intensity) * material_get_color(pos, r.mat).rgb + material_get_emission(pos, r.mat), 1);
}
...
float4 main( float4 position : SV_POSITION, float2 TexCoord : TEXCOORD ) : SV_TARGET
{
  ...
  sceneData.mainLight.pos = float3(12, 8, 0);
  sceneData.mainLight.color = float3(1.0, 1.0, 1.0);
  sceneData.mainLight.intensity = 1.0;
  sceneData.ambientLight = float3(0.2, 0.2, 0.2);
