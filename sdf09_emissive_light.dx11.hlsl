...
SdfResult scene_emissive(SceneData data, float3 p)
{
  ...
  float3    pLight2 = transform_translate(p, float3(15, 0, 0));
  float3    pBox = transform_translate(p, float3(-12, 2, 0));
  SdfResult sceneRes = 
    op_union(
      op_union(
      ...
      sd_box(pBox, float3(2,4,2), make_material(float4(0, 0, 0, 1), 0, data.mainLight.intensity * float4(0.2, 0.7, 0.1, 1)))
  );
...
float raymarch_to_target(SceneData sceneData, float3 pos, float3 dir, float maxDist, float lightSize = 1)
{
    float t = 0.05;
    float shadow = 1.0;
...
float4 compute_lighting(SceneData sceneData, float3 pos, float3 normal, SdfResult r)
{
  ...
  // Let's compute light from the emissives
  SdfResult emissive = scene_emissive(sceneData, pos);
  float     distanceToLight = emissive.dist;
  float3    toLight = compute_direction_to_emissive(sceneData, emissive, pos);
  float     dt = max(0, dot(toLight, normal));
  float     atten = 10 / (1 + distanceToLight * distanceToLight);
  
  float   shadow = raymarch_to_target(sceneData, pos, toLight, distanceToLight, 5);
  float3  diffuseLighting = atten * shadow * dt * material_get_emission(pos, emissive.mat);
...
float3 compute_absorption(SceneData sceneData, float3 currentTransmittance, float3 origin, float stepSize, float3 dir, float maxStep, out float3 scattering)
{
  ...
  {      
    ...
    float     shadow = 1;//raymarch_to_target(sceneData, p, to_emissive, emissive.dist);
...
{
  float3 absorption = 0 * float3(0.01, 0.008, 0.006);
  ...
  float     shadow = 1;//raymarch_to_target(sceneData, origin, to_emissive, emissive.dist);
...
float4 main( float4 position : SV_POSITION, float2 TexCoord : TEXCOORD ) : SV_TARGET
{
  ...
  sceneData.ambientLight = float3(0.05, 0.05, 0.05);
  ...
  if (i == MAX_ITER) col = float4(0,0,0,1);//plas(uv * 0.1, 0.1 * fGlobalTime);
