...
{
  float3 p1 = p;//transform_rotateY(p, fGlobalTime);
...
float3 compute_absorption(float3 p)
{
  float3 absorption = 3 * float3(0.01, 0.008, 0.006);
  
  // Aberration - brightness inside the shape
  //if (length(p) < 4) return float3(-0.5,-0.5,-0.5);
  ...
  if (length(p) < 4) 
  {
    ...
    // Use 3d noise
    return noise(2 * p + 25 * fGlobalTime * float3(0.2, 0.1, 0.05)) * 0.2;
  }
...
float4 main( float4 position : SV_POSITION, float2 TexCoord : TEXCOORD ) : SV_TARGET
{
  ...
  {
    SdfResult current = scene(sceneData, currentPos, false);
    if (current.dist < 1e-3)
    {
      // Got inside
    ...
    else
    {
      ...
      float3 absorption = compute_absorption(currentPos);
  ...
  // Fog
  col.rgb = col.rgb * transmittance + get_fog_color() * (1 - transmittance);
